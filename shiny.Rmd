...
**Version Note:** The behavior of `DT::renderDataTable` changed in version 0.0.65. Please update if you are using an older version.
</div>
...
Note `input$tableId_rows_selected` is the numeric indices of the selected rows when the table is processed on the client side. For server-side processing, there is no way to obtain the numeric indices because **DT** does not know the full table: only one page of the table data is returned from the server, and we have no idea the n-th row on a page is which row in the full table (by comparison, client-side tables fully live in the web browser). A workaround in this case is to use the row names as the indices. You must turn on the display of row names by `datatable(data, rownames = TRUE)` (in most cases, this is just the default behavior) if you want to get the names of selected rows from `input$tableId_rows_selected`.
</div>
</div>
...
- `input$tableId_rows_all`: the indices of rows on all pages (after the table is filtered by the search strings)
    ...
    - again, there is no way to know all the available data on the server side, `input$tableId_rows_all` will be the same as `input$tableId_rows_current` for tables using server-side processing
...
After a table has been rendered in a Shiny app, you can use the proxy object returned from `dataTableProxy()` to manipulate it. Currently supported methods are `selectRows()`, `selectColumns()`, and `addRow()`. See [this Shiny app](https://yihui.shinyapps.io/DT-proxy) for an example of using these methods to update a table.
