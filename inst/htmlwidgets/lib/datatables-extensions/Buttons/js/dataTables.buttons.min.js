    ...
    i.Api.register("buttons.exportData()", function(a) {
        if (this.context.length) {
            var b = new i.Api(this.context[0]),
                ...
                */
                var rawHeaderMatrix = getHeaders( b.settings()[0] );
			    var headerMatrix = [];
			    var numRows = rawHeaderMatrix.length;
		        for ( var rowIdx = 0;  rowIdx < numRows;  rowIdx++ ) {
		        	var headerRow = b.columns(c.columns).indexes().map( function (a) {
			             var $th = rawHeaderMatrix[rowIdx][a];
			             return c.format.header($th, a, $th);
			        } ).toArray();
			       headerMatrix.push( headerRow );
			    }
                /*END ADD*/
                var g = b.table().footer() ? b.columns(c.columns).indexes().map(function(a) {
                    var d =
                        b.column(a).footer();
