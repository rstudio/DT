...
```{r}
datatable(iris, extensions = list(KeyTable = list(initScroll = FALSE)))
```

We turned off the [`initScroll` option](http://datatables.net/extensions/keytable/options) above because we do not want this extension to scroll the page automatically to this section when this page is loaded.
