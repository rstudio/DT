...
- The option `searchDelay` is set to `1000` (1 second) by default;
...
The function `JS()` tells R that this element is not an ordinary character vector, but literal JavaScript code, which will be evaluated in the web browser. We need this special function because there is no way to encode a JavaScript function in R (using **jsonlite**) and decode it in the browser.
...
1. [UI language and internationalization](004-i18n.html)
1. [Search result highlighting](006-highlight.html)
1. [Search options (regular expressions and case-sensitive searching)](007-search.html)
1. [Disable searching for individual columns](009-searchable.html)
